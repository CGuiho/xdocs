...
- Runtime CLI dependency: Citty; descriptor and configuration content still use Bun-native TOML and YAML parsing without additional parser packages
...
### Citty Command Architecture

The CLI is defined by one declarative Citty command tree in `source/cli.ts`.
Citty is the only argument parser and command router. It owns root and nested
commands, short aliases, required strings/positionals, enum validation, default
and hidden routes, and ordinary command-specific usage.

`runCli(rawArgs)` uses Citty's library-safe `parseArgs`, `runCommand`, and
`renderUsage` APIs rather than the process-exiting `runMain` helper. This keeps
the exported CLI runner testable while executable entrypoints retain stable
process-level error handling. Usage errors are contextual and occur before
descriptor scans or agent-file mutation. `--help-tree` and `--help-docs` remain
extended xdocs renderers in `source/help.ts`; ordinary `-h`/`--help` output is
generated by Citty.
...
CLI parser internals are intentionally private. The Citty migration removed the
former `parseArgs`, `stringFlag`, `booleanFlag`, `listFlag`, and
`XDocsParsedArgs` exports. Programmatic callers that need CLI semantics should
use `runCli(rawArgs)`; domain callers should use the focused typed APIs below.
...
- `source/help.ts`: extended command-tree output, Markdown help docs, public help helpers, and version display; ordinary usage is rendered by Citty.
...
- Citty root/nested routing, aliases, generated help, required and enum values, contextual usage errors, hidden/default routes, structured output cleanliness, file outputs, init/agent operations, self-management dry runs, and public parser-API removal.
