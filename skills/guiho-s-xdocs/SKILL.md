...
Use the directory name when naming the descriptor, for example
`technologies/technologies.xdocs.md`. `.xdocs.md` by itself and legacy
`.docs.md` files are invalid descriptor names. A legacy `XDOCS.md` file is not
a descriptor or repository index. Every user-facing xdocs command removes that
legacy file from the effective project directory before performing its
...
non-excluded directory to arbitrary depth, retains every named descriptor, and reports malformed, orphaned, duplicate, or
...
locations and the bounded XDocs instruction block in the current
...
- The explicit `xdocs init` agent-resource bootstrap
  is a setup exception. It does not authorize arbitrary Markdown metadata
  maintenance.
